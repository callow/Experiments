...
import static security.credentialstorage.implementation.posix.keyring.GnomeKeyringLibrary.GNOME_KEYRING_ATTRIBUTE_SIZE;
import static security.credentialstorage.implementation.posix.keyring.GnomeKeyringLibrary.GNOME_KEYRING_ITEM_GENERIC_SECRET;
...
public abstract class GnomeKeyringBackedSecureStore<E extends StoredSecret> implements SecretStore<E> {
  ...
  protected static final Logger logger = LoggerFactory.getLogger(
      GnomeKeyringBackedSecureStore.class);
  ...
  protected <T> T readSecret(final String key, final BiFunction<String, char[], T> mapper) {
    if (INSTANCE != null && SCHEMA != null) {
      final Pointer searchAttributes = GLibLibrary.INSTANCE.g_array_new(0, 0,
          GNOME_KEYRING_ATTRIBUTE_SIZE);
      final Pointer[] foundList = new Pointer[1];
      try {
        // set attributes to search
        INSTANCE.gnome_keyring_attribute_list_append_string(searchAttributes, ATTRIBUTE_TYPE,
            getType());
        ...
        if (checkResult(result, "Could not find the item in storage.")) {
          if (foundList[0] != null && INSTANCE.g_list_length(foundList[0]) > 0) {
            final GnomeKeyringLibrary.GnomeKeyringFound item = INSTANCE.g_list_nth_data(
                foundList[0], 0);
            if (item != null && item.secret != null) {
              ...
              if (attrArray.len > 0) {
                ...
                final GnomeKeyringLibrary.GnomeKeyringAttribute dummyArray = new GnomeKeyringLibrary.GnomeKeyringAttribute(
                    attrArray.data);
                final GnomeKeyringLibrary.GnomeKeyringAttribute[] attrib = (GnomeKeyringLibrary.GnomeKeyringAttribute[]) dummyArray.toArray(
                    attrArray.len);
  ...
  protected int writeSecret(final String key, final String account, final char[] secret) {
    if (INSTANCE != null && SCHEMA != null) {
      return INSTANCE.gnome_keyring_store_password_sync(SCHEMA,
  ...
  protected int deleteSecret(final String key) {
    if (INSTANCE != null && SCHEMA != null) {
      return INSTANCE.gnome_keyring_delete_password_sync(SCHEMA, ATTRIBUTE_TYPE, getType(),
  ...
  private static GnomeKeyringLibrary.PointerToPointer getGnomeKeyringInfoStruct() {
    ...
    final int ret = INSTANCE.gnome_keyring_get_info_sync(GnomeKeyringLibrary.GNOME_KEYRING_DEFAULT,
        keyring_info_container);
  ...
  private static boolean isSimplePasswordAPISupported() {
    ...
    try {
      ...
      INSTANCE.gnome_keyring_find_password_sync(SCHEMA, pPassword,
  ...
  private static boolean isGnomeKeyringUnlocked(
      final GnomeKeyringLibrary.PointerToPointer keyring_info) {
    ...
    if (locked) {
      ...
      if (allowUnlock) {
        final int ret = INSTANCE.gnome_keyring_unlock_sync(
            GnomeKeyringLibrary.GNOME_KEYRING_DEFAULT, null);
  ...
  protected static boolean checkResult(final int retCode, final String message) {
    if (retCode != GnomeKeyringLibrary.GNOME_KEYRING_RESULT_OK) {
      ...
      try {
        logger.error("Return code: {} description: {}", retCode,
            INSTANCE.gnome_keyring_result_to_message(retCode));
