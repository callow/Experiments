...
import static security.credentialstorage.implementation.posix.libsecret.LibSecretLibrary.SECRET_SEARCH_LOAD_SECRETS;
import static security.credentialstorage.implementation.posix.libsecret.LibSecretLibrary.SECRET_SEARCH_UNLOCK;
...
import security.credentialstorage.implementation.posix.internal.GLibInitializer;
import security.credentialstorage.implementation.posix.internal.GLibLibrary;
import security.credentialstorage.implementation.posix.libsecret.LibSecretLibrary.GError;
...
public abstract class LibSecretBackedSecureStore<E extends StoredSecret> implements SecretStore<E> {
  ...
  protected <T> T readSecret(final String key, final BiFunction<String, char[], T> mapper) {
    if (INSTANCE != null && SCHEMA != null) {
      ...
      try {
        ...
        GLibLibrary.INSTANCE.g_hash_table_insert(searchAttributesHashTable,
            getPointer(ATTRIBUTE_TYPE), getPointer(getType()));
        GLibLibrary.INSTANCE.g_hash_table_insert(searchAttributesHashTable,
            getPointer(ATTRIBUTE_KEY), getPointer(key));

        // find the item
        Pointer item = INSTANCE.secret_service_search_sync(null, SCHEMA, searchAttributesHashTable,
  ...
  protected boolean writeSecret(final String key, final String account, final char[] secret,
      final PointerByReference error) {
    if (INSTANCE != null && SCHEMA != null) {
      return INSTANCE.secret_password_store_sync(SCHEMA, LibSecretLibrary.SECRET_COLLECTION_DEFAULT,
          // save to disk
          key, //display name
          new String(secret), null, error,
          //attributes list
          ATTRIBUTE_TYPE, getType(), ATTRIBUTE_KEY, key, ATTRIBUTE_ACCOUNT, account, null);
    }
  ...
  protected boolean deleteSecret(final String key, final PointerByReference error) {
    if (INSTANCE != null && SCHEMA != null) {
      return INSTANCE.secret_password_clear_sync(SCHEMA, null, error, ATTRIBUTE_TYPE, getType(),
          ATTRIBUTE_KEY, key, null);
    }
  ...
  private static boolean isSimplePasswordAPISupported() {
    ...
    try {
      final PointerByReference error = new PointerByReference();
      pPassword = INSTANCE.secret_password_lookup_sync(SCHEMA, null, error,
          ...
          ATTRIBUTE_TYPE, "NullType", ATTRIBUTE_KEY, "NullKey", null);
  ...
  private static boolean isDefaultCollectionUnlocked() {
    ...
    try {
      secretService = INSTANCE.secret_service_get_sync(LibSecretLibrary.SECRET_SERVICE_NONE, null,
          error);
      ...
      if (secretService != null && checkResult(error, "Cannot get service")) {
        final Pointer secretCollection = INSTANCE.secret_collection_for_alias_sync(secretService,
            LibSecretLibrary.SECRET_COLLECTION_DEFAULT, LibSecretLibrary.SECRET_COLLECTION_NONE,
            null, error);
        ...
        if (secretCollection != null && checkResult(error, "Cannot get collection by alias")) {
          ...
          if (locked) {
            logger.info(
                "Default collection is locked, most likely due to UI is unavailable or user logged in "
                    + "automatically without supplying a password.");

            final boolean allowUnlock = Boolean.parseBoolean(
                System.getProperty(ALLOW_UNLOCK_DEFAULT_COLLECTION));
            if (allowUnlock) {
              ...
              final int unlockedItemCount = INSTANCE.secret_service_unlock_sync(secretService,
                  objects, null, unlocked, error);
              return unlockedItemCount > 1 && checkResult(error,
                  "Could not unlock collection. Libsecret collection is not available.");
            } else {
              logger.info("Collection is locked and unavailable, please set variable {} to "
                      + "allow unlocking the keyring with a popup dialog.",
                  ALLOW_UNLOCK_DEFAULT_COLLECTION);
              return false;
            }
          }
    ...
    if (isLinux()) {
      try {
        ...
        try {
          ...
          logger.warn(
              "Glib not available -- user will see warnings printed on screen. Those warnings are "
  ...
  private static <T> T getStoredSecret(final GLibLibrary.GList listItem,
      final BiFunction<String, char[], T> mapper) {
    ...
    try {
      ...
      final Pointer userNameValue = GLibLibrary.INSTANCE.g_hash_table_lookup(attributesHashTable,
          getPointer(ATTRIBUTE_ACCOUNT));
  ...
  protected static boolean checkResult(final PointerByReference error, final String message) {
    if (error.getValue() != null) {
      ...
      logger.error(message + ": domain: {}, code: {}, description: {}", gError.domain, gError.code,
          gError.message);
      return false;
    }
