...
public final class LibSecretBackedTokenPairStore extends LibSecretBackedSecureStore<StoredTokenPair> {
  ...
  public boolean add(final String key, StoredTokenPair secret) {
    ...
    try {
      ...
      result = writeSecret(key + REFRESH_TOKEN, secret.getRefreshToken().getType().getDescription(),
  ...
  public boolean delete(String key) {
    ...
    try {
      ...
      result =
          accessResult && checkResult(accessError, "Could not delete access token from storage");
    } finally {
    ...
    try {
      ...
      result &=
          refreshResult && checkResult(refreshError, "Could not delete refresh token from storage");
    } finally {
      if (accessError.getValue() != null) {
        GLibLibrary.INSTANCE.g_error_free(accessError.getValue());
      }
